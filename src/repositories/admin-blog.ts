...
export class AdminBlogRepository {
  ...
  async list(options: { limit?: number; offset?: number } = {}) {
    ...
    return this.database.select().from(blogPosts).orderBy(asc(blogPosts.createdAt)).limit(limit).offset(offset);
  }
  async count() {
    const [row] = await this.database.select({ total: sql`count(*)` }).from(blogPosts);
