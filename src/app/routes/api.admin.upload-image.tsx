...
export async function action({ context, request }: Route.ActionArgs) {
  ...
  if (typeof catalog !== "string" || !catalog.trim()) return Response.json({ success: false, error: "A catalog title is required" }, { status: 400 });
  ...
  const { objectKey, publicId, version } = catalogPosterIdentity(catalog, extension);
  ...
  const media = await services.db.media.create({ objectKey, publicId, version, mimeType: value.type, byteSize: value.size, checksum });
