...
import { BroadcastIcon, PlayIcon, UsersIcon } from "@phosphor-icons/react";
...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  const total = await services.db.publicReads.countDiscussions();
  const totalPages = Math.max(Math.ceil(total / PAGE_SIZE), 1);
  const page = Math.min(requested, totalPages);

  const [discussions, latestList] = await Promise.all([
    services.db.publicReads.getAllDiscussions({
      limit: PAGE_SIZE,
      offset: (page - 1) * PAGE_SIZE,
    }),
    services.db.publicReads.getAllDiscussions({ limit: 1 }),
  ]);
  ...
  return { total, totalPages, page, discussions, latest, spotifyEmbedUrl };
}

const pageHref = (target: number) => (target <= 1 ? "/discussions" : `/discussions?page=${target}`);

export default function DiscussionsPage() {
  const { total, totalPages, page, discussions, latest, spotifyEmbedUrl } =
