...
import { eq, avg, isNotNull, desc, and, ne } from "drizzle-orm";
...
export async function getLatestUserReviews(): Promise<UserRating[]> {
  try {
    ...
    // Get unique user IDs
    const userIds = [...new Set(latestReviews.map((rating) => rating.userId))];
    ...
    console.error("Error fetching latest user reviews:", error);
    return [];
  }
}
